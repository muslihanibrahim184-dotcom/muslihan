...
export async function supplierPay(supplier, tutar, aciklama) {
  await sb.from("supplier_movements").insert({ supplier_id: supplier.id, aciklama: aciklama || "Ödeme", tutar, tip: "ödeme", tarih: todayISO() }).throwOnError();
  await sb.from("suppliers").update({ bakiye: Number(supplier.bakiye) - tutar }).eq("id", supplier.id).throwOnError();
  await addCash(`Ödeme: ${supplier.ad}`, "cikis", tutar).throwOnError();
}

// --- Satış ------------------------------------------------------------
export async function recordSale({ urun, adet, musteri, odeme, birimFiyat }) {
  ...
  const { data: sale, error } = await sb.from("sales").insert({
    tarih: todayISO(), urun_id: urun.id, urun_ad: urun.ad, adet, birim_fiyat: birim,
