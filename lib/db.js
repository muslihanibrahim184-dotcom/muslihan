...
// --- Ürünler ----------------------------------------------------------
const RENK_YOK = (e) => { const m = String(e?.message || "").toLowerCase(); return m.includes("renk") || m.includes("foto") || e?.code === "PGRST204"; };
const TEMIZLE = (o) => { const { renk, foto, ...kalan } = o; return kalan; };
export async function addProduct(p) {
  try { await sb.from("products").insert(p).throwOnError(); }
  catch (e) {
    if (RENK_YOK(e)) await sb.from("products").insert(TEMIZLE(p)).throwOnError();
    else throw e;
  }
}
export async function updateProduct(id, patch) {
  try { await sb.from("products").update(patch).eq("id", id).throwOnError(); }
  catch (e) {
    if (RENK_YOK(e)) await sb.from("products").update(TEMIZLE(patch)).eq("id", id).throwOnError();
    else throw e;
  }
}

// --- Ürün fotoğrafı --------------------------------------------------
// Telefondan gelen fotoğraf büyük olur; yüklemeden önce küçültülür.
function fotoKucult(file, maks = 1200, kalite = 0.82) {
  return new Promise((coz, hata) => {
    const okuyucu = new FileReader();
    okuyucu.onerror = () => hata(new Error("Fotoğraf okunamadı"));
    okuyucu.onload = () => {
      const img = new Image();
      img.onerror = () => hata(new Error("Fotoğraf açılamadı"));
      img.onload = () => {
        let { width: g, height: y } = img;
        if (g > maks || y > maks) { const o = Math.min(maks / g, maks / y); g = Math.round(g * o); y = Math.round(y * o); }
        const cv = document.createElement("canvas"); cv.width = g; cv.height = y;
        cv.getContext("2d").drawImage(img, 0, 0, g, y);
        cv.toBlob((b) => b ? coz(b) : hata(new Error("Fotoğraf dönüştürülemedi")), "image/jpeg", kalite);
      };
...
export async function uploadProductPhoto(file) {
  const blob = await fotoKucult(file);
  const yol = `urun-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.jpg`;
  const { error } = await sb.storage.from("urun-foto").upload(yol, blob, { contentType: "image/jpeg", upsert: true });
  if (error) {
    if (String(error.message || "").toLowerCase().includes("bucket")) throw new Error("Fotoğraf deposu (urun-foto) yok. Supabase → SQL Editor'da 'urun-foto.sql' dosyasını bir kez çalıştırın.");
    throw error;
  }
  return sb.storage.from("urun-foto").getPublicUrl(yol).data.publicUrl;
}
