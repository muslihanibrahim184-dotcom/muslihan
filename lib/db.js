...
// --- Ürünler ----------------------------------------------------------
const RENK_YOK = (e) => String(e?.message || "").toLowerCase().includes("renk") || e?.code === "PGRST204";
export async function addProduct(p) {
  try { await sb.from("products").insert(p).throwOnError(); }
  catch (e) {
    if (RENK_YOK(e) && "renk" in p) { const { renk, ...kalan } = p; await sb.from("products").insert(kalan).throwOnError(); }
    else throw e;
  }
}
export async function updateProduct(id, patch) {
  try { await sb.from("products").update(patch).eq("id", id).throwOnError(); }
    ...
    if (RENK_YOK(e) && "renk" in patch) { const { renk, ...kalan } = patch; await sb.from("products").update(kalan).eq("id", id).throwOnError(); }
    else throw e;
  }
}
